...
async function computeMaskBbox(maskBuffer) {
...
function padBbox(bbox, imgWidth, imgHeight, paddingRatio = 0.15) {
...
async function prepareImagePatch(imageBuffer) {
  const metadata = await sharp(imageBuffer).metadata();
  const w = metadata.width, h = metadata.height;
  const scale = Math.min(1, 512 / w, 512 / h);
  ...
  const { data } = await sharp(imageBuffer).resize(scaledW, scaledH, { fit: "fill", kernel: "lanczos3" }).removeAlpha().extend({
    top: padTop,
    bottom: padBottom,
    left: padLeft,
    right: padRight,
...
async function prepareMaskPatch(maskBuffer) {
  const metadata = await sharp(maskBuffer).metadata();
  const w = metadata.width, h = metadata.height;
  const scale = Math.min(1, 512 / w, 512 / h);
  const scaledW = Math.round(w * scale);
  ...
  const padTop = Math.floor((512 - scaledH) / 2);
  const padBottom = 512 - scaledH - padTop;
  const { data } = await sharp(maskBuffer).resize(scaledW, scaledH, { fit: "fill" }).greyscale().extend({
    top: padTop,
    bottom: padBottom,
    left: padLeft,
    right: padRight,
...
async function restorePatch(outputTensor, padInfo, originalWidth, originalHeight) {
  ...
  }).resize(originalWidth, originalHeight, { fit: "fill", kernel: "lanczos3" }).png().toBuffer();
}
async function createFeatherMask(maskBuffer, sigma = 3) {
  return sharp(maskBuffer).greyscale().blur(sigma).png().toBuffer();
}
async function featherBlend(aiBuffer, originalBuffer, featherMaskBuffer) {
...
async function tiledInpaint(cropImgBuf, cropMaskBuf, cropW, cropH, session, imgInputName, maskInputName, outName) {
  ...
  for (let row = 0; row < rows; row++) {
    for (let col = 0; col < cols; col++) {
      ...
      const tileImg = await sharp(cropImgBuf).extract({ left, top, width: tw, height: th }).png().toBuffer();
      const tileMask = await sharp(cropMaskBuf).extract({ left, top, width: tw, height: th }).png().toBuffer();
      ...
      const outTensor = results[outName].data;
      const tileResult = await restorePatch(outTensor, padInfo, tw, th);
      const { data: tilePx } = await sharp(tileResult).removeAlpha().raw().toBuffer({ resolveWithObject: true });
      for (let y = 0; y < th; y++) {
        for (let x = 0; x < tw; x++) {
          ...
          if (left > 0 && x < OVERLAP) wx = Math.min(1, x / HALF);
          if (left + tw < cropW && x > tw - OVERLAP) wx = Math.min(1, (tw - 1 - x) / HALF);
          if (top > 0 && y < OVERLAP) wy = Math.min(1, y / HALF);
          if (top + th < cropH && y > th - OVERLAP) wy = Math.min(1, (th - 1 - y) / HALF);
          const w = wx * wy;
...
parentPort?.on("message", async (msg) => {
  ...
  try {
    ...
    const cropImgBuf = await sharp(fullImgBuf).extract({ left: rectInfo.x, top: rectInfo.y, width: rectInfo.width, height: rectInfo.height }).png().toBuffer();
    const cropMaskBuf = await sharp(fullMaskBuf).extract({ left: rectInfo.x, top: rectInfo.y, width: rectInfo.width, height: rectInfo.height }).png().toBuffer();
